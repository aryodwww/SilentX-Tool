...
def HandleCli(options_list, options_hidden_list):
    parser = SilentXArgumentParser(description=f"{tool_name} {tool_version}", add_help=False, allow_abbrev=False)
    ...
    print(MainColor(f"{banner_silentx}\n"))
    ...
    if has_cli_args: HandleCli(options_list, options_hidden_list)
        ...
        if version_interface:
            while True:
                ...
                Slow(MainColor(f"{banner_silentx}\n"))
                Slow(menu)
                choice = input(MainColor(f" ┌──({white}{os_username}@silentx)─{red}[{white}") + path_folder_tool + MainColor(f"]\n └─{white}$ {reset}")).strip().lower()
                try:
                    if choice in ("h", "help"): 
